...
class TestHelpdeskTicket(TestHelpdeskTicketBase):
    ...
    def test_helpdesk_ticket_datetimes(self):
        ...
        self.ticket.write({"employee_id": self.env['hr.employee'].search([('user_id', '=', self.user.id)], limit=1).id})
        self.assertTrue(
    ...
    def test_ticket_with_team_stage(self):
        ...
        employee = None
        if not self.env['hr.employee'].search([('user_id', '=', self.user.id)], limit=1):
            employee = self.env['hr.employee'].create({
                'name': self.user.name,
                'user_id': self.user.id
            })
        else:
            employee = self.env['hr.employee'].search([('user_id', '=', self.user.id)], limit=1)
