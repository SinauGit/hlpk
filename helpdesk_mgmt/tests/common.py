...
class TestHelpdeskTicketBase(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        if not employee_model.search([('user_id', '=', cls.user_own.id)], limit=1):
            cls.employee_own = employee_model.create({
                'name': cls.user_own.name,
                'user_id': cls.user_own.id
            })
        else:
            cls.employee_own = employee_model.search([('user_id', '=', cls.user_own.id)], limit=1)
            
        if not employee_model.search([('user_id', '=', cls.user_team.id)], limit=1):
            cls.employee_team = employee_model.create({
                'name': cls.user_team.name,
                'user_id': cls.user_team.id
            })
        else:
            cls.employee_team = employee_model.search([('user_id', '=', cls.user_team.id)], limit=1)
            
        if not employee_model.search([('user_id', '=', cls.user.id)], limit=1):
            cls.employee = employee_model.create({
                'name': cls.user.name,
                'user_id': cls.user.id
            })
        else:
            cls.employee = employee_model.search([('user_id', '=', cls.user.id)], limit=1)
