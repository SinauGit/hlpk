...
class HelpdeskTicket(models.Model):
    ...
    def _compute_employee_id(self):
        for ticket in self:
            if not ticket.employee_id and ticket.team_id:
                # Mencari employee yang terkait dengan alias_user_id
                if ticket.team_id.alias_user_id:
                    employee = self.env['hr.employee'].search([
                        ('user_id', '=', ticket.team_id.alias_user_id.id)
                    ], limit=1)
                    if employee:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if vals.get("employee_id") and not vals.get("assigned_date"):
